...
void LinkedList::add_start(int value)
{
    ...
    Node<int> *second_item = head->get_next();
    head->set_next(new Node<int>(head->get_value()));
...
void LinkedList::add_end(int value)
{
    ...
    while (prev->get_next() != nullptr)
        prev = prev->get_next();
    prev->set_next(new Node<int>(value));
